%==============================================
% WorldTheory.pl for actor sdm
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatusdm).	%%old version (deprecated)
actorobj(qatusdm).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor sdm
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of sdm" ). 
...
%-------------------------------------------------
%  User static rules about sdm
%------------------------------------------------- 
cell( c1,free).
cell( c2,free).
createMission( MATERIAL,SOURCE,LGV,mission( order( MATERIAL,SOURCE,LGV),DEST)):-findDestInWharehouse( MATERIAL,DEST),assert( mission( order( MATERIAL,SOURCE,LGV),DEST)).
findDestInWharehouse( MATERIAL,CELL):-retract( cell( CELL,free)),assert( cell( CELL,bookedfor( MATERIAL))).
/*
------------------------------------------------------------------------
